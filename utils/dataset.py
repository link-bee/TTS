...
from English.en_hparams import en_hparams as en_hps
...
class KoreanDataset(Dataset):
    ...
    def __getitem__(self, index):
        text, mel = self.f_list[index]
        text, mel = get_mel_text_pair(text, mel, 'korean')
...
class EnglishDataset(Dataset):
    ...
    def __getitem__(self, index):
        text, mel = self.f_list[index]
        text, mel = get_mel_text_pair(text, mel, 'english')
        return text, mel
...
def get_mel_text_pair(text, wav_path, language):
    text = get_text(text, language)
...
def get_text(text, language):
    if language == 'english':
        return torch.IntTensor(t2s_en(text, en_hps.text_cleaners))
    else:
        return torch.IntTensor(t2s_ko(text))
